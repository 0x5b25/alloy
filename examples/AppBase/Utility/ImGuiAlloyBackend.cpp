...
class ImGuiAlloyBackend {
    ...
    void _CreateDeviceObjects(const ImGui_ImplAlloy_InitInfo&);

    void _CreateRenderPipeline(const ImGui_ImplAlloy_InitInfo&);
    void _CreateBuffers(const ImGui_ImplAlloy_InitInfo&);
    void _CreateFontsTexture(const ImGui_ImplAlloy_InitInfo&);
...
void ImGuiAlloyBackend::_SetupRenderState( ImDrawData* draw_data,
    ...
    // Our visible imgui space lies from draw_data->DisplayPos (top left) to draw_data->DisplayPos+data_data->DisplaySize (bottom right).
    {
        ...
        //mvpBuffer->UnMap();
        auto pcSpan = std::span<uint32_t>((uint32_t*)mvp, 16);
    ...
    command_list->SetIndexBuffer(indexBuffer, indexFormat);
}
...
void ImGuiAlloyBackend::RenderDrawData(
    ...
    auto indexBuffer = _allocator.Allocate(requiredVertBufferSize, 4);
    ...
    _SetupRenderState(draw_data, &renderPass, vertBuffer, indexBuffer);

    // Setup render state structure (for callbacks and custom texture bindings)
    ImGuiPlatformIO& platform_io = ImGui::GetPlatformIO();
    ImGui_ImplAlloy_RenderState render_state { };
    render_state.pBackend = this;
    render_state.pRenderPass = &renderPass;
...
void ImGuiAlloyBackend::_CreateRenderPipeline(const ImGui_ImplAlloy_InitInfo& initInfo) {
    ...
    //}
    {
        auto& elem = resLayoutDesc.pushConstants.emplace_back();
        elem.bindingSlot = 0;
        elem.sizeInDwords = 16;
    }
    ...
    psoDesc.attachmentState.sampleCount = initInfo.msaaSampleCount;
    ...
    // Create the vertex shader
    {
        ...
        sd.enableDebug = true;
        auto vs = gd->GetResourceFactory().CreateShader(sd, g_VSMain);
    ...
    // Create the pixel shader
    {
        ...
        sd.enableDebug = true;
        auto fs = gd->GetResourceFactory().CreateShader(sd, g_PSMain);
...
void ImGuiAlloyBackend::_CreateBuffers(const ImGui_ImplAlloy_InitInfo&) {
...
void ImGuiAlloyBackend::NotifyEndFrame() {
    _allocator.NotifyFrameComplete();
}

// Backend data stored in io.BackendRendererUserData to allow support for multiple Dear ImGui contexts
// It is STRONGLY preferred that you use docking branch with multi-viewports (== single Dear ImGui context + multiple windows) instead of multiple Dear ImGui contexts.
static ImGuiAlloyBackend* ImGui_ImplAlloy_GetBackendData()
{
    return ImGui::GetCurrentContext() ? (ImGuiAlloyBackend*)ImGui::GetIO().BackendRendererUserData : nullptr;
}


bool ImGui_ImplAlloy_Init(const ImGui_ImplAlloy_InitInfo& info) {
    ImGuiIO& io = ImGui::GetIO();
    IMGUI_CHECKVERSION();
    IM_ASSERT(io.BackendRendererUserData == nullptr && "Already initialized a renderer backend!");

    // Setup backend capabilities flags
    ImGuiAlloyBackend* bd = IM_NEW(ImGuiAlloyBackend)(info);
...
void ImGui_ImplAlloy_RenderDrawData(ImDrawData* draw_data, alloy::IRenderCommandEncoder& renderPass) {
    ImGuiAlloyBackend* bd = ImGui_ImplAlloy_GetBackendData();
